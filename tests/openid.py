...
UTF8_URL = "http://example.com/François"
UTF8_HASH = "5c323abb04553a28f44490b21effdf8f4a9878d07775c9a3c3f4a9ec9a95ff33"


def test_simple_url() -> None:
    assert libravatar_url(openid=COMMON_OPENID) == (
        COMMON_PREFIX_HTTP + COMMON_HASH
    )
    assert (
        libravatar_url(openid=COMMON_OPENID, https=True)
        == COMMON_PREFIX_HTTPS + COMMON_HASH
    )
    assert (
        libravatar_url(openid=COMMON_OPENID, https=False)
        == COMMON_PREFIX_HTTP + COMMON_HASH
    )


def test_openid_hashing() -> None:
    # equivalent URLs
    assert (
        libravatar_url(openid="http://example.COM/id")
        == COMMON_PREFIX_HTTP + COMMON_HASH
    )
    assert (
        libravatar_url(openid="HTTP://example.com/id", https=True)
        == COMMON_PREFIX_HTTPS + COMMON_HASH
    )

    # different URLs
    assert (
        libravatar_url(openid="https://example.com/id")
        == COMMON_PREFIX_HTTP
    ...
    # UTF-8 in URLs
    assert libravatar_url(openid=UTF8_URL) == COMMON_PREFIX_HTTP + UTF8_HASH
    assert (
        libravatar_url(openid=UTF8_URL, https=True)
        == COMMON_PREFIX_HTTPS + UTF8_HASH
    )


def test_other_params() -> None:
    assert (
        libravatar_url(openid=COMMON_OPENID, default="/local.png")
        == COMMON_PREFIX_HTTP + COMMON_HASH + "?d=%2Flocal.png"
    )
    assert (
        libravatar_url(
            openid=COMMON_OPENID, size=150, https=True, default="mm"
        )
...
def test_user_identity() -> None:
    assert parse_user_identity(None, COMMON_OPENID) == (
        COMMON_HASH,
        COMMON_DOMAIN,
    )

    assert parse_user_identity(None, "http://example.COM/id") == (
        COMMON_HASH,
        COMMON_DOMAIN,
    )
    assert parse_user_identity(None, "  HTTP://example.com/id  ") == (
    ...
    assert parse_user_identity(
        None, "http://user:password@Example.com/id"
    ) == (
    ...
    assert parse_user_identity(None, "http://openid.example.COM/id") == (
